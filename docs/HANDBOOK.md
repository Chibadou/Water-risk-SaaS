...
- **BNPE / prélèvements** (`lib/bnpe.ts` + `/api/bnpe` + `BnpePanel`, Sprint 10) : Hub'Eau `/v1/prelevements/chroniques?code_commune_insee=…` → `aggregateBnpe` somme les volumes par usage sur l'année la plus récente (usages normalisés en catégories, volumes ≤ 0 ignorés). **Bloc informatif seulement — hors score composite** (pas de dénominateur ressource à la bonne échelle). Vérifié réel : Chartres 819 072 m³, Toulouse 62 Mm³ (2023). Test : `npx tsx scripts/test/bnpe.test.ts`.
