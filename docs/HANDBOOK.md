...
1. **Reste du Sprint 9** (reporté, chacun = vrai chantier) : IPS nappes (≥15 ans de chroniques + stats de référence), débits vs VCN10/QMNA5 (Hydroportail — pas d'API JSON libre propre), rattachement des stations par sous-bassin/aquifère (`code_bdlisa`, référentiels Sandre), trancher ZAS Sandre vs périmètre VigiEau.
